...
void dfs(int u){
    ...
    for( int i = 0; i < a[u].size(); i++){
        int v = a[u][i]; // v la dinh ke cua u
        if( v == p[u]) continue; // bo qua dinh cha
        if( num[v]){ //neu v la dinh to tien hay (u, v) la canh nguoc
